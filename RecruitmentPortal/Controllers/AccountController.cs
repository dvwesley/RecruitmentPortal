...
namespace RecruitmentPortal.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult CreateOrUpdate(string returnAction, string returnController, bool userId)
        {
            var user = TempData["User"] as ApplicationUser;
            if (user != null)
            {
                var dbUser = UserManager.FindByName(user.UserName);
                if(dbUser != null)
                {
                    user.Id = dbUser.Id;
                    dbUser.FirstName = user.FirstName;
                    dbUser.LastName = user.LastName;
                    dbUser.MiddleInitial = user.MiddleInitial;
            ...
            if (userId)
            {
                return RedirectToAction(returnAction, returnController, new { userId = user.Id });
            }
            else
            {
        ...
        public ActionResult Edit(ProfileViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ViewBag.StatusMessage = "Profile details updated successfully.";
                }
                else
                {
                    AddErrors(result);
